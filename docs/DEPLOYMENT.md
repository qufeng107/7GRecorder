...
CLI does not provide a hard bytes-per-second limiter. COS uploads original publish parts without transcoding or archive
wrapping. Legacy `COS_COMPRESSION_*` entries in an existing server environment are ignored after this change and may
be removed during a later configuration cleanup.
