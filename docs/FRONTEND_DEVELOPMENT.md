...
以及录制/账号编辑不被轮询重置。系统设置的存储/TLS 表单已加入内存草稿、保存反馈、放弃修改、路由离开确认和刷新/关闭提醒；后台刷新及旧保存响应不会覆盖新修改。
上传 Bilibili/COS 与歌曲设置已接入同一草稿机制；切换录制配置需要确认放弃草稿，保存期间禁止切换。
录制配置/账号编辑弹窗支持键盘关闭、焦点约束及放弃确认，凭证创建和未应用剪辑也有离开提醒。
COS 禁用接口仅保存禁用状态，其他字段保留为未保存草稿；启用后保存或放弃修改可结束编辑。
草稿不写入 localStorage/sessionStorage，登出或会话失效时清除。后台刷新失败保留已加载页面，并允许重试。
