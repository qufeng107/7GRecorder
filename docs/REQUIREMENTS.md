...
前端保留 React 技术栈，按业务模块拆分控制台，页面使用独立路由；公开主播宣传页采用独立布局，
允许丰富动画和创意交互，动画依赖按需加载。共享 UI、主题、开发测试模式与迁移顺序见 `FRONTEND_UI.md`。
...
前端迭代要求支持本地开发与隔离测试环境，优先建立可复现的本地隔离环境。远程 staging 的部署位置另行确定；`dev` 仍只跑 CI，不自动部署。
...
- OpenLive JSONL 使用本地托管总配额中的固定 2 GiB 子配额；当前写入分片永不删除，已关闭分片按创建顺序从旧到新
  滚动删除原文，直到回到 2 GiB 以内；会话计数和采集质量 metadata 继续保留；
