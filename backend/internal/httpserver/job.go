package httpserver

import (
	"errors"
	"net/http"

	"github.com/7grecorder/7grecorder/backend/internal/account"
	"github.com/7grecorder/7grecorder/backend/internal/config"
	"github.com/7grecorder/7grecorder/backend/internal/db"
	"github.com/7grecorder/7grecorder/backend/internal/job"
	"github.com/gogf/gf/v2/frame/g"
	"github.com/gogf/gf/v2/net/ghttp"
)

func bindJobHandlers(cfg config.Config, s *ghttp.Server) {
	s.BindHandler("/api/v1/jobs", func(r *ghttp.Request) {
		if !requireMethod(r, http.MethodGet) {
			return
		}
		withJobStore(r, cfg, func(actor account.User, store job.Store) {
			items, err := store.List(r.Context(), actor, r.Get("limit").Int())
			if err != nil {
				writeJobError(r, err)
				return
			}
			r.Response.WriteJson(g.Map{"items": items, "page": 1, "page_size": len(items), "total": len(items)})
		})
	})

	s.BindHandler("/api/v1/jobs/{id}", func(r *ghttp.Request) {
		if !requireMethod(r, http.MethodGet) {
			return
		}
		id := r.Get("id").Int64()
		withJobStore(r, cfg, func(actor account.User, store job.Store) {
			item, err := store.Get(r.Context(), actor, id)
			if err != nil {
				writeJobError(r, err)
				return
			}
			r.Response.WriteJson(item)
		})
	})

	s.BindHandler("/api/v1/jobs/{id}/actions/retry", func(r *ghttp.Request) {
		if !requireMethod(r, http.MethodPost) {
			return
		}
		id := r.Get("id").Int64()
		withJobStore(r, cfg, func(actor account.User, store job.Store) {
			item, err := store.Retry(r.Context(), actor, id)
			if err != nil {
				writeJobError(r, err)
				return
			}
			r.Response.WriteJson(item)
		})
	})

	s.BindHandler("/api/v1/jobs/{id}/actions/cancel", func(r *ghttp.Request) {
		if !requireMethod(r, http.MethodPost) {
			return
		}
		id := r.Get("id").Int64()
		withJobStore(r, cfg, func(actor account.User, store job.Store) {
			item, err := store.Cancel(r.Context(), actor, id)
			if err != nil {
				writeJobError(r, err)
				return
			}
			r.Response.WriteJson(item)
		})
	})
}

func withJobStore(r *ghttp.Request, cfg config.Config, fn func(account.User, job.Store)) {
	database, err := db.Open(r.Context(), cfg)
	if err != nil {
		writeAPIError(r, http.StatusInternalServerError, "DATABASE_UNAVAILABLE", "Database is unavailable.", nil)
		return
	}
	defer database.Close()

	actor, err := account.NewStore(database).UserBySessionToken(r.Context(), r.Cookie.Get(sessionCookieName).String())
	if errors.Is(err, account.ErrNotFound) || errors.Is(err, account.ErrDisabledUser) {
		writeAPIError(r, http.StatusUnauthorized, "NOT_AUTHENTICATED", "Login is required.", nil)
		return
	}
	if err != nil {
		writeAPIError(r, http.StatusInternalServerError, "SESSION_LOOKUP_FAILED", "Session lookup failed.", nil)
		return
	}
	fn(actor, job.NewStore(database))
}

func writeJobError(r *ghttp.Request, err error) {
	switch {
	case errors.Is(err, job.ErrForbidden):
		writeAPIError(r, http.StatusForbidden, "FORBIDDEN", "Job operation is not allowed.", nil)
	case errors.Is(err, job.ErrNotFound):
		writeAPIError(r, http.StatusNotFound, "JOB_NOT_FOUND", "Job was not found.", nil)
	case errors.Is(err, job.ErrValidation):
		writeAPIError(r, http.StatusBadRequest, "VALIDATION_FAILED", "Job request is invalid for the current state.", nil)
	default:
		writeAPIError(r, http.StatusInternalServerError, "JOB_OPERATION_FAILED", "Job operation failed.", nil)
	}
}
