...
export function createConsoleFixtures() {
  ...
  return (
    ...
    if ((path.endsWith("/timeline") || path.endsWith("/raw-files")) && path.startsWith("/api/v1/live-analytics/sessions/")) return ok({ items: [], truncated: false });
    ...
    if (path === "/api/v1/storage/local")
      return ok({
        ...
        live_analytics_bytes: 1048576,
        live_analytics_files: 2,
        live_analytics_max_bytes: 2147483648,
        managed_local_bytes: 103448576,
