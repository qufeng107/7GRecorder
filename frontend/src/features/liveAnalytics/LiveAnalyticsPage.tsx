...
export default function LiveAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    setAppID(config.data.app_id ? String(config.data.app_id) : "");
    setCredentialID(config.data.credential_id ?? 0);
  ...
  const save = useMutation({
    mutationFn: () => requestJson<LiveAnalyticsConfig>(`/api/v1/recording-profiles/${profileID}/live-analytics`, {
      method: "PUT", body: JSON.stringify({ app_id: Number(appID), credential_id: credentialID, enabled }),
    }),
    onSuccess: () => {
      void client.invalidateQueries({ queryKey: ["live-analytics-config", profileID] });
  ...
  return <div className="feature-page space-y-6">
    ...
    <section className="console-card space-y-4 p-5">
      ...
      <label className="grid gap-2 text-sm">{en ? "Recording profile" : "录制配置"}<select className="console-input" value={profileID} onChange={(event) => setProfileID(Number(event.target.value))}>{(profiles.data?.items ?? []).map((profile) => <option key={profile.id} value={profile.id}>{profile.name} · {profile.room_id}</option>)}</select></label>
      ...
      <Button disabled={save.isPending || !profileID || !appID || !credentialID} onClick={() => save.mutate()}>{save.isPending ? (en ? "Saving…" : "正在保存…") : (en ? "Save collector" : "保存采集配置")}</Button>
