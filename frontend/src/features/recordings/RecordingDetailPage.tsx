...
type List<T> = { items: T[]; total: number; truncated?: boolean };
...
export default function RecordingDetailPage() {
  ...
  const sessions = useQuery({ queryKey: ["live-analytics-sessions", source.data?.recording_profile_id, id], queryFn: () => requestJson<List<LiveCaptureSession>>(`/api/v1/live-analytics/sessions?profile_id=${source.data?.recording_profile_id}&upload_source_id=${id}`), enabled: Boolean(source.data?.recording_profile_id), refetchInterval: 5000 });
  ...
  const relatedSessions = sessions.data?.items ?? [];
  return <div className="feature-page space-y-6">
    ...
    <Section title={en ? "Capture quality" : "采集质量"}>{sessions.isError ? <PageStatus retry={() => void sessions.refetch()} /> : sessions.isPending ? <PageStatus loading /> : null}{sessions.data?.truncated ? <p>{en ? "Showing up to 100 overlapping connections; older connections may be omitted." : "最多显示 100 个重叠连接，更早的连接可能未显示。"}</p> : null}{relatedSessions.length ? <div className="space-y-2 text-sm">{relatedSessions.map((session) => <div key={session.id} className="rounded-md border border-border p-3"><p className="font-medium"><Link className="text-accent underline" to={`/admin/live-analytics/sessions/${session.id}`}>{session.status} #{session.id}</Link> · {session.event_count} {en ? "events" : "个事件"}</p><p className="mt-1 text-muted">{en ? "Raw evidence" : "原始证据"}: {session.raw_status} · {formatBytes(session.raw_size_bytes)}{session.raw_deleted_at ? ` · ${en ? "cleaned" : "已清理"} ${session.raw_deleted_at}` : ""}</p><p className="mt-1 text-muted">{en ? "Unknown" : "未知事件"}: {session.unknown_event_count} · {en ? "Gaps" : "采集缺口"}: {session.gap_count} · {en ? "Last event" : "最后事件"}: {session.last_event_at || "—"}</p></div>)}</div> : sessions.isSuccess ? <p className="text-sm text-muted">{en ? "No overlapping OpenLive capture session was found. Missing collection is not counted as zero engagement." : "未找到时间重叠的 OpenLive 采集场次；缺少采集不会按零互动处理。"}</p> : null}</Section>
  </div>;
}
...
function AnalyticsPending({ en, sessions }: { en: boolean; sessions: LiveCaptureSession[] }) { return <p className="text-sm text-muted">{sessions.length ? (en ? "Capture records are available below. Detailed analysis requires retained raw evidence." : "可在下方采集记录中查看事件；详细分析需要尚未清理的原始证据。") : (en ? "No collected data is available for this time range." : "该时间范围暂无已采集数据。")}</p>; }
